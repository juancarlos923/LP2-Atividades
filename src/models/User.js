...
async function consultar_imagem(id) {
  const db = await Database.connect();
  ...
  const user = await db.all(sql,[id]);
  return user;
}

export default { create, read, readByEmail, token_verificado_email, createNewUser, confirmVerifyEmail, consultar_imagem };
